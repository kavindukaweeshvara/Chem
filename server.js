...
app.get('/student/courses/:courseId/lessons', studentAuth, async (req, res) => {
    ...
    if (dbConnected) {
        try {
            ...
            const [enrollment] = await db.query(`SELECT * FROM enrollments WHERE user_id=$1 AND course_id=$2 AND status='active' AND payment_status='verified'`, { bind: [userId, courseId] });
...
app.get('/admin/courses/:courseId/lessons', adminAuth, async (req, res) => {
    ...
    let lessonList = '';
    if (lessons.length > 0) { lessons.forEach(l => { lessonList += `<div class="lesson-card"><div><strong style="color:#1a237e;">📖 ${l.title}</strong><span style="background:#e3f2fd;color:#1565c0;padding:3px 10px;border-radius:12px;font-size:12px;margin-left:10px;">${l.topic_name || ''}</span>${l.zoom_link ? '<br><small style="color:#28a745;">📡 Zoom: ' + l.zoom_link.substring(0,50) + '...</small>' : ''}${l.video_url ? '<br><small style="color:#2563eb;">🎬 Video: ' + l.video_url.substring(0,50) + '...</small>' : ''}</div><div><a href="/admin/lessons/delete/${l.id}?courseId=${courseId}" class="btn-small btn-danger" onclick="return confirm('Delete?')">🗑️</a></div></div>`; }); } else { lessonList = '<p style="color:#666;text-align:center;padding:20px;">No lessons yet.</p>'; }
