        ...
        db.authenticate()
            .then(async () => {
                ...
                try {
                    ...
                    const [existing] = await db.query(`SELECT * FROM users WHERE username = 'Buddika'`);
                    if (existing.length === 0) {
                        const hashedPassword = await require('bcryptjs').hash('Buddika@2024', 12);
                        await db.query(
                            `INSERT INTO users (student_id, username, email, password, full_name, mobile_number, role, is_active) 
                             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
                            {
                        ...
                        console.log('✅ Admin user created: Buddika / Buddika@2024');
                    } else {
                        console.log('✅ Admin user already exists');
                    }
                    
                    console.log('✅ All tables & admin ready!');
                } catch (tableError) {
