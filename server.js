...
async function getTeacherImage() {
    ...
    try {
        ...
        if (a.length > 0 && a[0].profile_image) return `<img src="/uploads/${a[0].profile_image}" style="width:55px;height:55px;border-radius:50%;object-fit:cover;">`;
    } catch(e) {}
...
// FORGOT PASSWORD POST - GMAIL SMTP
    ...
    try {
        ...
        try {
            ...
            await transporter.sendMail({
                from: `"${TEACHER.name} LMS" <${process.env.EMAIL_USER || process.env.EMAIL_FROM || 'noreply@chemistry.lk'}>`,
                to: email,
                subject: 'Password Reset Code - ' + TEACHER.name,
                html: `<div style="max-width:500px;margin:0 auto;padding:30px;font-family:Arial,sans-serif;background:#f5f5f5;border-radius:10px"><div style="text-align:center;font-size:50px;margin-bottom:20px">⚗️</div><h2 style="color:#1a237e;text-align:center">Password Reset Code</h2><p style="color:#666;text-align:center;font-size:16px">Hello, ${users[0].full_name}!</p><p style="color:#666;text-align:center">Your code is:</p><div style="background:#1a237e;color:white;padding:20px;border-radius:10px;text-align:center;font-size:36px;font-weight:bold;letter-spacing:10px;margin:20px 0">${code}</div><p style="color:#666;text-align:center;font-size:13px">⚠️ Expires in 1 hour.</p><hr style="border:1px solid #e0e0e0;margin:20px 0"><p style="text-align:center;color:#1a237e;font-weight:bold">👨‍🏫 ${TEACHER.name}</p></div>`
            });
            emailSent = true;
...
app.post('/reset-password', async (req, res) => {
    try {
        ...
        if (password.length < 6) return res.send(`<script>alert('Min 6 characters!');window.location.href='/reset-password?token=${token}&verified=true'</script>`);
        if (dbConnected) { const [users] = await db.query(`SELECT * FROM users WHERE reset_token=$1 AND reset_token_expires > NOW()`, { bind: [token] }); if (users.length === 0) return res.send(`<script>alert('Expired!');window.location.href='/forgot-password'</script>`); const hash = await bcrypt.hash(password, 12); await db.query(`UPDATE users SET password=$1, reset_token=NULL, reset_token_expires=NULL, verification_code=NULL WHERE reset_token=$2`, { bind: [hash, token] }); }
        res.send(`<!DOCTYPE html><html><head><title>Updated</title><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:linear-gradient(135deg,#28a745,#218838);display:flex;justify-content:center;align-items:center;min-height:100vh;padding:20px}.box{background:white;padding:35px;border-radius:15px;box-shadow:0 10px 30px rgba(0,0,0,0.2);width:100%;max-width:400px;text-align:center}h2{color:#28a745;margin-bottom:10px}p{color:#666;margin-bottom:20px}.btn{display:inline-block;padding:12px 25px;background:#1a237e;color:white;text-decoration:none;border-radius:8px;font-weight:bold}</style></head><body><div class="box"><h2>✅ Password Updated!</h2><p>Changed successfully.</p><a href="/login" class="btn">🔐 Login Now</a></div></body></html>`);
...
app.get('/student/courses', studentAuth, async (req, res) => {
    ...
    let cc = '';
    if (courses.length > 0) { courses.forEach(c => { cc += `<div class="course-card"><h3>📚 ${c.title}</h3><p style="color:#666;">${c.description||''}</p><p style="font-size:13px;color:#999;">Rs.${c.price||0}</p><a href="/student/courses/${c.id}/lessons" class="btn">📖 View Lessons</a></div>`; }); } else { cc = '<p style="text-align:center;color:#666;padding:30px;">No courses yet.</p>'; }
...
app.get('/admin/enrollments', adminAuth, async (req, res) => {
    ...
    let tr = ''; if (enrollments.length > 0) { enrollments.forEach(e => { const sc = e.status==='active'?'#28a745':e.status==='revoked'?'#dc3545':'#ffc107'; const pc = e.payment_status==='verified'?'#28a745':e.payment_status==='pending_verification'?'#17a2b8':'#dc3545'; tr += `<tr><td><strong>${e.student_id}</strong></td><td>${e.full_name}</td><td>${e.mobile_number}</td><td>${e.course_title}</td><td><span style="background:${sc};color:white;padding:4px 10px;border-radius:12px;font-size:12px;">${e.status}</span></td><td><span style="background:${pc};color:white;padding:4px 10px;border-radius:12px;font-size:12px;">${e.payment_status}</span></td><td>${e.payment_proof?'<a href="'+e.payment_proof+'" target="_blank">📎</a>':'N/A'}</td><td>${e.payment_status==='pending_verification'?`<a href="/admin/enrollments/verify/${e.id}" class="btn-mini btn-approve" onclick="return confirm('Approve?')">✅</a>`:''}${e.status==='active'?`<a href="/admin/enrollments/revoke/${e.id}" class="btn-mini btn-revoke" onclick="return confirm('Revoke?')">❌</a>`:e.status==='revoked'?`<a href="/admin/enrollments/activate/${e.id}" class="btn-mini btn-approve">🔄</a>`:''}<a href="/admin/enrollments/delete/${e.id}" class="btn-mini btn-delete" onclick="return confirm('Delete?')">🗑️</a></td></tr>`; }); } else { tr = '<tr><td colspan="8" style="text-align:center;padding:30px;">No enrollments.</td></tr>'; }
...
app.get('/health', (req, res) => res.json({ status: 'ok', db: dbConnected ? 'connected' : 'disconnected' }));
